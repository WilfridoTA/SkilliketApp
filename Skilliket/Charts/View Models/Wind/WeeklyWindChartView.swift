...
struct WeeklyWindChartView: View {
    ...
    var selectedDateValue: (week: Date, velocity: Float)? {
        if let rawSelectedDate {
            return windViewModel.windByWeek.first(where: {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            // Title for the chart
            Text("Weekly Wind Velocities (m/s)")
                .font(.title2)
                .bold()
                .padding(.bottom, 5)
    ...
    var selectionPopover: some View {
        if let rawSelectedDate, let selectedDateValue {
            VStack {
                ...
                Text("\(String(format: "%.2f", selectedDateValue.velocity)) m/s")
                    .bold()
...
@available(macOS 14.0, iOS 17, *)
#Preview {
    WeeklyWindChartView(windViewModel: .preview)
