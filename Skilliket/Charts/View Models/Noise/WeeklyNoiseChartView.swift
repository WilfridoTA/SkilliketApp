...
struct WeeklyNoiseChartView: View {
    ...
    var selectedDateValue: (week: Date, level: Float)? {
        if let rawSelectedDate {
            return noiseViewModel.noiseByWeek.first(where: {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            // Title for the chart
            Text("Weekly Noise Levels (dB)")
                .font(.title2)
                .bold()
                .padding(.bottom, 5)
            
            // The chart displaying weekly noise data
            Chart(noiseViewModel.noiseByWeek, id: \.week) {
                BarMark(
                    x: .value("Week", $0.week, unit: .weekOfYear),
                    y: .value("Noise Level", $0.level)
                )
    ...
    var selectionPopover: some View {
        if let rawSelectedDate, let selectedDateValue {
            VStack {
                ...
                Text("\(String(format: "%.2f", selectedDateValue.level)) dB") // Changed from "m/s" to "dB"
                    .bold()
...
@available(macOS 14.0, iOS 17, *)
#Preview {
    WeeklyNoiseChartView(noiseViewModel: .preview)
