...
class WaterViewModel: ObservableObject{
    ...
    @Published var waterData: [WaterData2]
    ...
    var waterByWeekday: [(number: Int, level: [WaterData2])] {
    ...
    // Initializers
    init(waterData: [WaterData2] = [], lastTotalLevel: Float = 0.0) {
        self.waterData = waterData
        self.lastTotalLevel = lastTotalLevel
    }
    
    convenience init() {
        self.init(waterData: WaterData2.higherWeekendThreeMonthsExamples, lastTotalLevel: 2000.0)
    }
    
    // MARK: - Helper Functions
    
    // Group wind data by day
    func waterGroupedByDay(water: [WaterData2]) -> [Date: [WaterData2]] {
        var waterByDay: [Date: [WaterData2]] = [:]
    ...
    func waterGroupedByWeek(water: [WaterData2]) -> [Date: [WaterData2]] {
        var waterByWeek: [Date: [WaterData2]] = [:]
        let calendar = Calendar.current
        for entry in water {
    ...
    func waterGroupedByMonth(water: [WaterData2]) -> [Date: [WaterData2]] {
        var waterByMonth: [Date: [WaterData2]] = [:]
        let calendar = Calendar.current
        for entry in water {
    ...
    func waterGroupedByWeekday(water: [WaterData2]) -> [Int: [WaterData2]] {
        var waterByWeekday: [Int: [WaterData2]] = [:]
        let calendar = Calendar.current
        for entry in water {
    ...
    func totalLevelPerDate(waterByDate: [Date: [WaterData2]]) -> [(day: Date, level: Float)] {
    ...
    func totalLevelPerWeek(waterByWeek: [Date: [WaterData2]]) -> [(week: Date, level: Float)] {
    ...
    func totalLevelPerMonth(waterByMonth: [Date: [WaterData2]]) -> [(month: Date, level: Float)] {
    ...
    func averageLevelPerNumber(waterByNumber: [Int: [WaterData2]]) -> [(number: Int, level: Double)] {
    ...
    func histogram(for levels: [WaterData2], bucketSize: Int) -> [(bucket: Int, count: Int)] {
    ...
    static var preview: WaterViewModel {
        ...
        vm.waterData = WaterData2.higherWeekendThreeMonthsExamples
