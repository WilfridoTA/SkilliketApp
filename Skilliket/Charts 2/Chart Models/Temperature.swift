...
import Foundation
struct TemperatureData2: Identifiable, Equatable, Codable {
    let id: UUID
    let celsius: Float
    ...
    let time: Date
     ...
     // Example instance
     static var example = TemperatureData2(
     id: UUID(),
     celsius: 22.0,
     waterLevel: 1.7,
     velocity: 5.0,
     ...
     static var examples: [TemperatureData2] = [
     TemperatureData2(id: UUID(), celsius: 18.0, waterLevel: 1.7, velocity: 4.5, noiseLevel: 45.0, date: Date(timeIntervalSinceNow: -7_200_000), time: Date()),
     TemperatureData2(id: UUID(), celsius: 20.5, waterLevel: 2.4, velocity: 5.0, noiseLevel: 50.0, date: Date(timeIntervalSinceNow: -14_400_000), time: Date()),
     TemperatureData2(id: UUID(), celsius: 15.0, waterLevel: 2.5, velocity: 3.5, noiseLevel: 48.0, date: Date(timeIntervalSinceNow: -21_600_000), time: Date()),
     TemperatureData2(id: UUID(), celsius: 17.5, waterLevel: 1.2, velocity: 4.0, noiseLevel: 46.0, date: Date(timeIntervalSinceNow: -28_800_000), time: Date()),
     TemperatureData2(id: UUID(), celsius: 21.0, waterLevel: 1.8, velocity: 6.0, noiseLevel: 49.0, date: Date(timeIntervalSinceNow: -36_000_000), time: Date())
     ]
     
     // Generate random temperature data for the last three months
     static func threeMonthsExamples() -> [TemperatureData2] {
     let threeMonthsAgo = Calendar.current.date(byAdding: .month, value: -3, to: Date())!
     
     let exampleTemperatureData: [TemperatureData2] = (1...300).map { _ in
     ...
     static var higherWeekendThreeMonthsExamples: [TemperatureData2] = {
     let threeMonthsAgo = Calendar.current.date(byAdding: .month, value: -3, to: Date())!
     let exampleTemperatureData: [TemperatureData2] = (1...300).map { _ in
     let randomDate = Date.random(in: threeMonthsAgo...Date())
