...
struct NoiseData: Identifiable, Equatable, Codable {
    let id: UUID
    ...
    /*
     ...
     static var examples: [NoiseData] = [
     NoiseData(id: UUID(), noiseLevel: 45.3, waterLevel: 1.2, velocity: 4.5, celsius: 20.0, date: Date(timeIntervalSinceNow: -7_200_000), time: Date()),
     NoiseData(id: UUID(), noiseLevel: 55.8, waterLevel: 1.8, velocity: 5.2, celsius: 21.0, date: Date(timeIntervalSinceNow: -14_400_000), time: Date()),
     NoiseData(id: UUID(), noiseLevel: 49.5, waterLevel: 2.0, velocity: 5.8, celsius: 22.5, date: Date(timeIntervalSinceNow: -21_600_000), time: Date()),
     NoiseData(id: UUID(), noiseLevel: 60.2, waterLevel: 0.9, velocity: 3.5, celsius: 19.5, date: Date(timeIntervalSinceNow: -28_800_000), time: Date()),
     NoiseData(id: UUID(), noiseLevel: 42.1, waterLevel: 1.6, velocity: 6.0, celsius: 23.0, date: Date(timeIntervalSinceNow: -36_000_000), time: Date())
     ]
     
     // Generates noise level data for the last three months
     static func threeMonthsExamples() -> [NoiseData] {
     let threeMonthsAgo = Calendar.current.date(byAdding: .month, value: -3, to: Date())!
     
     let exampleNoiseData: [NoiseData] = (1...300).map { _ in
     ...
     static var higherWeekendThreeMonthsExamples: [NoiseData] = {
     let threeMonthsAgo = Calendar.current.date(byAdding: .month, value: -3, to: Date())!
     let exampleNoiseData: [NoiseData] = (1...300).map { _ in
     let randomDate = Date.random(in: threeMonthsAgo...Date())
...
class NoiseJSON: Codable {
    let noiseArray: [NoiseData]

    init(noiseArray: [NoiseData]) {
    ...
    required init(from decoder: Decoder) throws {
        ...
        self.noiseArray = try container.decode([NoiseData].self, forKey: .noiseArray)
    }

    // Manual Encodable conformance (optional, if you also want to encode)
    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(noiseArray, forKey: .noiseArray)
    }
